...
def train_rf_real(ticker:str = "GSPC", seed: int | None = 42) -> None:
    print("##################################################")
    print(f"Random Forest Baesline = Real data - {ticker}")
    ...
    preds_scaled = rf.predict(X_test_flat).reshape(-1,1)
    ...
    log_results("RF_Baseline", ticker, {**return_metrics, **price_metrics}, path=RESULTS_FILE)
    
    print("Random Forest Baseline real data done.")
    print("##################################################\n")
    
def train_rf_synthetic(series_name: str = "linear_gradual_drift", series_number: int = 1, seed: int | None = 42) -> None:
    print("##################################################")
    print(f"Random Forest Baesline = Synthetic data - {series_name} - Series {series_number}")
    ...
    log_results(model_name="RF_Baseline", dataset=f"{series_name}_{series_number}", metrics=metrics, path=RESULTS_FILE)

    print("Random Forest Baseline synthetic data done.")
    print("##################################################\n")        
    

def main():
    for i in tqdm(range(1,31),desc="Training RF on Baseline Real data", ncols=100):
        train_rf_real(ticker="GSPC", seed=i)
    ...
    for name in synthetic_series:
        for i in tqdm(range(1,31),desc=f"Training RF on {name} synthetic data", ncols=100):
            train_rf_synthetic(series_name=name, series_number=i, seed=i)
